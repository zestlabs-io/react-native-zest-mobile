...
export class AuthBridge {
  ...
  constructor(config) {
    this._authDB = new PouchDB('_auth', { adapter: 'react-native-sqlite' });
    this._config = config;
  ...
  loadAuthFromDB = async () => {
    ...
    try {
      ...
      console.log('Loaded auth', JSON.stringify(doc));
  ...
  _onAuthData = (data) => {
    ...
    throw Error('No data loaded ' + JSON.stringify(data) + ' ' + new Date(data.accessTokenExpirationDate));
  };
